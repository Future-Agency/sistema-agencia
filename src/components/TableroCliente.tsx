...
import { supabase, type Cliente, type Owner, type FaseCliente, type Nota, type Reporte } from '@/lib/supabase'
import { SemaforoIcon, Badge } from './ui'

type Props = { cliente: Cliente; owners: Owner[]; onBack: () => void; onUpdate: () => void; showToast: (msg: string, type: 'success' | 'error') => void }

export default function TableroCliente({ cliente, owners, onBack, onUpdate, showToast }: Props) {
  ...
  async function updateFase(faseId: number, newStatus: string) {
    ...
    await supabase.from('fases_cliente').update({ status: newStatus, semaforo: sm[newStatus] }).eq('id', faseId)
    loadDetails()
  }
  ...
  return (
    <div className="fade-in">
      ...
      {tab === 'proceso' && (
        <div className="card">
          <div className="card-header"><span style={{ fontWeight: 600 }}>Fases del {cliente.is_onboarding ? 'Onboarding' : 'Ciclo Ongoing'}</span></div>
          <div className="card-body">
            {fases.map(f => (
              <div key={f.id} className="fase-row">
                ...
                <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                  ...
                  {f.status !== 'pending' && <SemaforoIcon color={f.semaforo} />}
                  <span style={{ fontSize: 11, color: '#6a6a80' }}>{f.dias_default}d</span>
                </div>
              </div>
            ))}
          </div>
